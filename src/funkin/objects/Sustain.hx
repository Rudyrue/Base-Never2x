	...
	override public function drawComplex(camera:FlxCamera) {
		...
		for (i in 0...sustainDivisions) {
			...
			if (Math.min(Note.finalVertices[1], Note.finalVertices[5]) <= camera.viewMarginBottom && Math.max(Note.finalVertices[1], Note.finalVertices[5]) >= camera.viewMarginTop)
				camera.drawNoteVertices(holdFrame, Note.finalVertices, colorTransform, blend, holdAntialiasing, false, colorTransform.alphaMultiplier * coyoteAlpha);
		}
		...
		if (Math.min(Note.finalVertices[1], Note.finalVertices[5]) <= camera.viewMarginBottom && Math.max(Note.finalVertices[1], Note.finalVertices[5]) >= camera.viewMarginTop)
			camera.drawNoteVertices(tailFrame, Note.finalVertices, colorTransform, blend, antialiasing, false, colorTransform.alphaMultiplier * coyoteAlpha);
